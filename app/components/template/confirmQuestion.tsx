...
const ConfirmQuestion = memo(
  ({ questionsData }: { questionsData: QuestionType[] }) => {
    const [turn, setTurn] = useState(0);
    const [questions, setQuestions] = useState(questionsData);
    const [question, setQuestion] = useState<string>(questions[turn]?.question);
    const [answerOne, setAnswerOne] = useState<string>(questions[turn]?.answerOne);
    const [answerTwo, setAnswerTwo] = useState<string>(questions[turn]?.answerTwo);
    const [answerThree, setAnswerThree] = useState<string>(
      questions[turn]?.answerThree
    );
    const [answerFour, setAnswerFour] = useState<string>(questions[turn]?.answerFour);
    const [category, setCategory] = useState<string>(questions[turn]?.category);
              ...
              <div className="w-full flex justify-center">
                <div className="grid grid-cols-3 gap-2">
                  ...
                  <div
                    ...
                    className={`w-40 h-12 bg-first/5 text-first/70 text-[1.3rem] rounded-lg flex items-center justify-center active:scale-[99%] ${
                      category === "بازی های یارانه ای" &&
                      "borderSelectCategory"
                    }`}
                  >
                    بازی های یارانه ای
                  </div>
